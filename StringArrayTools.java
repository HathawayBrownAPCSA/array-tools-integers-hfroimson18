...
public class StringArrayTools
{
  ...
  */
  ...
  public void fillKeyboard ()
  {
    ...
    System.out.println ("Please enter one string on a line, separated by spaces.");
    System.out.println ("Use an empty string to signal the end of the list.");

    arrayCount = 0;
    String n = keyboard.nextLine();
    while (n.length() != 0)                 // stop input when user enters a string with length 0
    {
      addItem(n);
      n = keyboard.nextLine();
    }
  ...
  // -------------------- METHODS TO WORK WITH THE ARRAY --------------------------
   ...
   /** Finds the largest element in a String array
  ...
  /** Finds the index of largest element in a string array.
    * @return the index of the largest element in a string array. */
  public int getMaxIndex()
  {
    int maxIndex = 0;
    String maxNum = arrayData[0];
    for (int i = 1; i < arrayCount; i++)
    {
      if (arrayData[i].compareTo(maxNum) > 0)
      {
        maxNum = arrayData[i];
        maxIndex = i;
      }
  ...
  /** Finds the index of the smallest element in a string array.
    * @return the index of the smallest element in a string array. */
  public int getMinIndex()
  {
    int minIndex = 0;
    String minNum = arrayData[0];
    for (int i = 1; i < arrayCount; i++)
      if (arrayData[i].compareTo(minNum) < 0)
    {
        minNum = arrayData[i];
        minIndex = i;
    }
